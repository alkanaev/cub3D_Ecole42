...
void	*mlx_int_recupe_xpm(void *xvar,void *info,int info_size,char *(*f)(), int *width, int *height)
{
  int	pos;
  char	*line;
  char	**tab;
...
void	*mlx_xpm_file_to_image(void *xvar,char *file,int *width,int *height)
{
  ...
  img = mlx_int_recupe_xpm(xvar,ptr,size,mlx_int_get_line, width, height);
  munmap(ptr,size);
  close(fd);
  return (img);
}
...
{
  return (mlx_int_recupe_xpm(xvar,xpm_data,0,mlx_int_static_line, width, height));
}
